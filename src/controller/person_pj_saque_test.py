...
class MockPersonPj:
    ...
    def get_person_pj_by_id(self, person_id: int):
    ...
    def saque_person_pj(self, person_id: int, valor: float):
        """Simula saque diretamente, se necessário"""
        if person_id != 1:
            return {"errors": {"message": "Conta PJ não encontrada"}}
        ...
        return {
            "data": {
                "id": self.person.id,
                "attributes": {
                    "nome_fantasia": self.person.nome_fantasia,
